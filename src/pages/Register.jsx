...
import { registroAPI } from '../config/api';
...
const Register = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // El registro hace login automático, así que el token ya está guardado
    ...
    <div className="min-h-screen bg-gradient-to-br from-pardos-cream via-white to-pardos-cream flex items-center justify-center p-15">
      <div className="w-full max-w-7xl grid md:grid-cols-2 gap-20 items-center">
        ...
        <div className="bg-pardos-rust rounded-[2.7rem] p-10 shadow-2xl w-full max-w-3xl mx-auto">
          ...
          {error && (
            <div className="bg-red-600 text-white px-6 py-4 rounded-lg mb-6 text-center font-lato">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-7">
            ...
            <button
              type="submit"
              className="w-full bg-pardos-brown hover:bg-pardos-purple text-white font-spartan font-bold py-6 text-2xl rounded-full transition-all duration-300 transform hover:scale-105 shadow-lg mt-8 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
              disabled={loading}
            >
